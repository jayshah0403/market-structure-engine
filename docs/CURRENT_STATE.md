...
**3a. Download.** `open_archive_csv(url)` streams the response to a `tempfile.TemporaryFile` in 1 MB chunks and opens the zip from there. The fetch is bounded twice: `ARCHIVE_TIMEOUT_SECONDS` (60 s) is handed to `requests` as the connect and read timeout, and `ARCHIVE_DEADLINE_SECONDS` (300 s) is a wall-clock deadline checked on every chunk, started before the request so it covers connecting and waiting for headers too. The first catches a server that goes silent; the second catches one that trickles a chunk just inside the read timeout forever, which would otherwise hold an API compute slot open indefinitely. Both raise `ArchiveUnavailable`. Streaming to disk rather than into a `BytesIO` is deliberate: `zipfile` needs a seekable source, and a 5M-row day is a ~75 MB zip around a ~430 MB CSV. The URL comes from `archive_url_template` (`{symbol}` / `{date}`) — no URL is written in the code. HTTP 404 → `SessionNotPublished` (the archive appears ~02:00 UTC the next day); any other HTTP failure, a connection error, a corrupt zip, or a zip that does not hold exactly one member → `ArchiveUnavailable`.
...
**Latency and its bound.** A hit is milliseconds. A miss is roughly 20 s — the archive download and aggregation. There is **no request timeout at the API layer**; a cold compute is bounded by the engine's two archive constants (§3a): 60 s per read, and a 300 s total-transfer deadline. Either raises `ArchiveUnavailable` → 503 rather than holding one of the three compute slots open. The "90 s request budget" named in earlier drafts was never enforced anywhere and has been dropped from the spec.

**The range endpoint never computes.** It reads the cache and reports every uncached day in `missing`. A day cached at a stale `engine_version` is reported as missing too, since it is not a hit and this route cannot refresh it; requesting that date individually recomputes it. The window is inclusive and capped at **365 days** (`MAX_RANGE_DAYS`); wider is a 422. Since the route cannot compute, the cap bounds response size and the `missing` list rather than work.
...
**`tests/test_compute.py`** — PR 3 acceptance. 36 tests: 26 pure, 9 needing network + a database, 1 slow.
...
- Archive fetch bounds: the request carries `timeout=ARCHIVE_TIMEOUT_SECONDS` and `stream=True`; a connect timeout and a mid-download read timeout both surface as `ArchiveUnavailable` with nothing written. Regression guards — they pin behaviour that already existed, since nothing else stops the timeout being dropped and a stalled download then holding an API compute slot.
- The total-transfer deadline: a response that drips prompt chunks forever — never tripping the read timeout — is abandoned once `ARCHIVE_DEADLINE_SECONDS` passes, with nothing written. The constant is shrunk to 0.05 s in the test so it costs milliseconds, but the chunks are produced over real time, so the production path is what runs. Its counterpart serves a real zip through a fake response and asserts the deadline does **not** fire on a legitimate archive — the only test that drives `open_archive_csv` end to end (download → zip → CSV) without the network.
...
**`tests/test_api.py`** — PR 4 acceptance. 31 tests, all pure: `db`'s five reads and `ingest.compute_day` are replaced by `FakeStore`, an in-memory stand-in, so the whole file runs in CI with no `CONNECTION_STRING` and no network.
...
- Range: returns cached rows and names the missing dates, never computes, counts a stale row as missing, serves a window exactly at the 365-day cap and rejects one a day wider with a 422.
...
- ~~No cap on the range window~~ — **closed in PR 4 review.** Capped at 365 days inclusive (`MAX_RANGE_DAYS`), 422 beyond. Independent of V2_SPEC D6's max lookback of 90, which is scoped to PR 5's endpoints.
...
- ~~The 90 s cold-path budget is not enforced by the app~~ — **resolved in PR 4 review by dropping it.** It was an aspiration no layer implemented. The real bound is the engine's `ARCHIVE_TIMEOUT_SECONDS` (60 s) on the archive request, now recorded in V2_SPEC PR 4 and pinned by regression tests; the route separately caps the wait for a compute *slot* at 30 s.
- ~~The archive timeout bounds stalls, not slow transfers~~ — **closed in PR 4 review.** `ARCHIVE_DEADLINE_SECONDS` (300 s) is a wall-clock deadline across the whole fetch, checked on every chunk, so the slow-drip case is bounded too. 300 s was chosen as roughly fifteen times the ~20 s a real 5–20 MB archive takes: generous for a slow link, still a ceiling. Both bounds are pinned by tests.
